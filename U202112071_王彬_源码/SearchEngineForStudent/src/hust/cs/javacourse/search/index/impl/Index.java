...
public class Index extends AbstractIndex {
    ...
    public void addDocument(AbstractDocument document) {
        ...
        for(AbstractTermTuple tup:document.getTuples()){
            if (!map.containsKey(tup.term)){
                map.put(tup.term,new ArrayList<>());
            }
            map.get(tup.term).add(tup.curPos);
        }
        // 更新索引
        for (AbstractTerm k:map.keySet()){
            if (this.termToPostingListMapping.get(k)==null)
                this.termToPostingListMapping.put(k,new PostingList());
            this.termToPostingListMapping.get(k).add(
                    new Posting(document.getDocId()
                            , map.get(k).size()
                            , map.get(k)));
        }
    }
    ...
    /**
     * <pre>
     * 对索引进行优化，分别对索引里每个单词的PostingList按docId从小到大排序
    ...
    public void optimize() {
        for (AbstractPostingList p:this.termToPostingListMapping.values()){
